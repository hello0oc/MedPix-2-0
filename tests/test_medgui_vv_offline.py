...
def test_parse_json_object_round_trips_trial_profile_schema(app_module):
    """parse_json_object must correctly round-trip the full trial profile schema."""
    import json
    profile = {
        "topic_id": "mpx1009",
    ...
    parsed = app_module.parse_json_object(text)
    assert parsed["topic_id"] == "mpx1009"
    assert parsed["key_facts"][0]["notes"] == "imaging_source: direct_image_analysis"
...
# ── validate_and_coerce_trial_profile tests ─────────────────────────────────

def _make_raw_trial(overrides=None):
    """Build a minimal parseable trial profile dict for testing."""
    base = {
        "topic_id": "WRONG-UID",
        "profile_text": "## Clinical History\nSome history.",
        "key_facts": [
...
def test_validate_coerce_forces_topic_id_lowercase(app_module):
    raw = _make_raw_trial({"topic_id": "WRONG-UID"})
    result = app_module.validate_and_coerce_trial_profile(raw, "MPX1009", used_images=True)
    assert result["topic_id"] == "mpx1009"


def test_validate_coerce_null_coercion_filler_strings(app_module):
    raw = _make_raw_trial()
    # Inject a filler string on demographics
    raw["key_facts"].insert(0, {
        "field": "demographics",
        "value": "N/A",
        "evidence_span": None,
        "required": True,
    ...
    result = app_module.validate_and_coerce_trial_profile(raw, "mpx1009", used_images=False)
    demo_fact = next(kf for kf in result["key_facts"] if kf["field"] == "demographics")
    assert demo_fact["value"] is None
    # demographics must be added to missing_info
    missing_fact = next(kf for kf in result["key_facts"] if kf["field"] == "missing_info")
    assert "demographics" in missing_fact["value"]


def test_validate_coerce_injects_imaging_findings_when_absent(app_module):
    """When the model omits imaging_findings entirely it must be injected."""
    raw = _make_raw_trial()
    # Remove imaging_findings from key_facts
    raw["key_facts"] = [
        kf for kf in raw["key_facts"] if kf["field"] != "imaging_findings"
    ]
    result = app_module.validate_and_coerce_trial_profile(raw, "mpx1009", used_images=False)
    img_fact = next(
        (kf for kf in result["key_facts"] if kf["field"] == "imaging_findings"), None
    )
    assert img_fact is not None
    assert img_fact["value"] is None
    assert "ehr_extracted" in (img_fact["notes"] or "")
    missing_fact = next(kf for kf in result["key_facts"] if kf["field"] == "missing_info")
    assert "imaging_findings" in missing_fact["value"]


def test_validate_coerce_imaging_source_annotation_preserved(app_module):
    raw = _make_raw_trial()
    result = app_module.validate_and_coerce_trial_profile(raw, "mpx1009", used_images=True)
    img_fact = next(kf for kf in result["key_facts"] if kf["field"] == "imaging_findings")
    assert "imaging_source" in (img_fact["notes"] or "")


def test_build_trial_profile_prompt_appends_topic_id(app_module):
    base = app_module.TRIAL_PROFILE_PROMPT_TEXT_ONLY
    prompt = app_module.build_trial_profile_prompt(base, "MPX1009")
    assert prompt.startswith(base)
    assert "topic_id: mpx1009" in prompt
    text = (
        "**Diagnosis:** Old diagnosis\n"
        "**Final Diagnosis:** New diagnosis\n"
        "**Rationale:** Why"
